.PHONY: setup install up down dev clean help init-db reset-db dbt-deps generate check-env
...
help:
	...
	@echo "  make setup        - Install dependencies, check .env, start Docker (local), and initialize database"
	@echo "  make .env         - Create configuration file from template"
...
setup: check-env install infra-check init-db dbt-deps

check-env:
	@if [ ! -f .env ]; then \
		echo "❌ Error: .env file is missing."; \
		echo "👉 Run 'make create-env' to create it."; \
		echo "👉 Then EDIT it if you are running remotely (JupyterHub) to set STARROCKS_HOST."; \
		echo "👉 Finally, run 'make setup' again."; \
		exit 1; \
	fi
...
create-env:
	...
	@echo ""
	@echo "👉  If you are running REMOTELY (e.g. JupyterHub):"
	@echo "    Edit .env NOW and set STARROCKS_HOST to your remote database IP."
	@echo ""
	@echo "👉  If you are running LOCALLY (Docker):"
	@echo "    You can proceed."
	@echo ""
	@echo "🚀  Next step: Run 'make setup'"
	@echo "--------------------------------------------------------------------"
